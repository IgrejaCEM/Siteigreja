...
async function forceContent() {
  try {
    ...
    const result = await client.query(
      'UPDATE settings SET homeContent = $1, homeCss = $2, updated_at = NOW() WHERE id = 3',
      [defaultContent, '']
    );
    ...
    const check = await client.query('SELECT homeContent FROM settings WHERE id = 3');
